...
class InteractiveSimulation:
    ...
    def _setup_main_plot(self):
        ...
        ax.set_title(f"3I/ATLAS: Loeb's Fixed-Axis Jet [{mode_label}]", fontsize=16, fontweight="bold", color="white", pad=15)
    ...
    def _setup_pass_fail_panel(self):
        ...
        self.pass_fail_text = self.fig.text(
            0.76, 0.28,
            panel_text,
            fontsize=7, color="#aaaacc", ha="left", va="top", family="monospace",
            bbox=dict(boxstyle="round,pad=0.4", facecolor="#1a1a2e", edgecolor="#444466", alpha=0.95)
        )
    ...
    def select_claim(self, claim_id: int):
        ...
        if hasattr(self, 'claim_results') and claim_id in self.claim_results:
            ...
            self.desc_title.set_text(f"#{claim_id}: {claim['title']}")
            self.desc_text.set_text("Click [i] for full analysis")
